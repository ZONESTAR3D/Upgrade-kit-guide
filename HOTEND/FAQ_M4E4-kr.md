...
M4 핫엔드와 E4 핫엔드는 필라멘트 로딩에 대한 요구 사항이 다릅니다.
- **필라멘트 로딩 수량 요구사항:**
   - M4 핫엔드는 단색 또는 다중 색상 인쇄 여부에 관계없이 동시에 4개의 필라멘트를 로드해야 합니다.
   - E4 핫엔드는 인쇄된 색상에 따라 로드할 필라멘트 수를 선택할 수 있습니다.
- **필라멘트 로딩 위치 요구 사항:**
   - M4 핫엔드는 핫엔드 하단에 필라멘트를 설치해야 합니다.
   - E4 핫 엔드는 핫 엔드의 깔때기 입구 위에 필라멘트를 설치해야 합니다(PTFE 튜브를 약 20mm 연장).
### M4 핫엔드용
#### M4 핫엔드에 필라멘트를 로드하는 단계:
##### [![](https://img.youtube.com/vi/-47yB95uIxI/0.jpg)](https://www.youtube.com/watch?v=-47yB95uIxI)
M4 핫엔드에 필라멘트를 로드할 때 프린팅 중에 사용해야 하는 압출기 수에 관계없이 ***필라멘트 4개를 모두 핫엔드에 로드해야 하며*** 모든 필라멘트가 제대로 작동하는지 확인하는 것이 중요합니다. 인쇄를 시작하기 전에 핫엔드*** 하단에 로드합니다.     
:warning: ***단색 또는 다중 색상으로 인쇄하든 상관없이 4개의 필라멘트를 모두 M4V6 핫엔드에 로드해야 합니다.***
1. 필라멘트를 압출기와 핫엔드에 장착하기 전에 대각선 펜치로 필라멘트의 앞부분을 자릅니다.
2. 모든 Extruder에 4개의 필라멘트를 하나씩 장착한 후 필라멘트가 핫엔드의 내부 PTFE 튜브에 들어갈 때까지 Extruder의 기어를 회전시킨 후 4~5바퀴 더 회전한 후 정지합니다.

#### M4 핫엔드에서 필라멘트를 언로드하는 단계:
1. 노즐을 가열(PLA는 200℃ / PETG/ABS는 230℃)하고 온도가 도달할 때까지 기다립니다. ***Menu: Prepare>>Filament>>Preheat: 200/230***
